    ...
    */
    ...
    public function setFolderPath($folderPath)
    {
        ...
        if ((mb_strlen($folderPath) > 512)) {
            throw new \InvalidArgumentException('invalid length for $folderPath when calling FolderSummary., must be smaller than or equal to 512.');
        }
        if ((mb_strlen($folderPath) < 1)) {
            throw new \InvalidArgumentException('invalid length for $folderPath when calling FolderSummary., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^\/.*$/", ObjectSerializer::toString($folderPath)))) {
            throw new \InvalidArgumentException("invalid value for \$folderPath when calling FolderSummary., must conform to the pattern /^\/.*$/.");
        }
